...
const float stop_rel_point_x      = 0.65;
...
float Function::get_relative_control_point(vector<Point3d> point3)
{
   ...
   if( point3.size()>0)
   {
     ...
     relative_obstacle_point3.y = min_pose.y;// point3.at(num).y
...
float get_chance_value(float obstacles_x_one,float obstacles_y_one,float mid_y)
 {
      ...
      {
              if(dx_one<0.6){
                  R  = pow(dy_one,0.9) + pow(dx_one,0.9);//2
                //  oFile_init<<"dx_one R = "<<R<<endl;
                  cout<<"dx_one<0.6 R = "<<R<<endl;
              }
              else{
                if(dx_one>1.0)
                  R  = (pow(dy_one,5) + pow(dx_one,5))*1.4;//
                else{
                  R  = 1.7;//pow(dy_one,0.5) + pow(dx_one,0.5)
                }
...
//      oFile_init<< "dx_one"<< " "<<dx_one<<"  "<< "dy_one"<< " "<<dy_one<<"  "<<" x = "<<robot_pose.x<<"  "<<" R = "<<R<<"  "<<" K = "<<K<<endl;
    ...
    //  K = K1;
      ...
      cout<<" 测试point_x = "<<point_x<<" point_y = "<<point_y<<"  flag = "<<flag<<endl;
      if(((flag == 1)&&(point_x<(stop_rel_point_x+0.15)))||((point_x<stop_rel_point_x)&&(abs(point_y)<stop_rel_point_y)))
      {
        state = 2;
        flag = 1;
...
//      cout<<"obstacles_x_two = "<<obstacles_x_two<<"obstacles_y_two = "<<obstacles_y_two<<endl;
      ...
      if(flag == 0)
        state = 0; 
      cout<<" relative_obstacle_point3.y = "<<relative_obstacle_point3.y<<" obstacles_lidar_z = "<<obstacles_lidar_z<<" obstacles_num = "<<obstacles_num<<endl;
...
//    get_max_obstacle_x      = 0;
  ...
  //  float min_y = 10;
    ...
    for(int i = 90;i<=270;)//ros::spinOnce();加入此代码可以进入调阅函数
      ...
      oFile_init<< "OOOKKKKKK"<< " "<<" x = "<<robot_pose.x<<" y = "<<robot_pose.y<<endl;
    //  cout<<"get_max_obstacle_x = "<<get_max_obstacle_x<<endl;
    //Result_pose min_pose = obstacle_fun.get_min_x_y(_Relative_point3);
...
{ 
    ...
    int Max_index = 0;
    oFile_init.open("Processing_lidar_data_11.csv",ios::out|ios::trunc);
