   ...
   if(dy<0.5)
    {
          if((dx<0.6)||(dx>1.6)){
              ...
              cout<<"dx<0.6 R = "<<R<<endl;
                  ...
                  R  = 1.8;// 1.5 pow(dy_one,0.5) + pow(dx_one,0.5)
                }
              //  oFile_init<<"else R = "<<R<<endl;
                  cout<<"else  R = "<<R<<endl;
                //  if(R<1.7) R = 1.7;
        ...
        if(((get_distance_angle[i] > _min_lidar_date)&&(max_dis<1.5))||((flag_max_dis==1)&&(max_dis<1.6)&&(get_distance_angle[i] > _min_lidar_date)))
         {
           // state = 1;
            ...
            {
              //  /2.0 改进角度
              float Chose_XY_A = (P_i-abs(robot_pose.angle))*(robot_pose.angle/abs(robot_pose.angle))/2.0;
              obstacle_x = robot_pose.x+get_distance_angle[i]*sin(P_i/2.0-Chose_XY_A+(((i-180)/180.0)*P_i));
              obstacle_y = robot_pose.y+get_distance_angle[i]*cos(P_i/2.0-Chose_XY_A+(((i-180)/180.0)*P_i));
            }
            else{
